"""
streamlit/views/home.py
LocID Native App — Home (Dashboard)

Status dashboard displayed as the app's default landing view:
  - License status card
...
def _load_home_data(_session_id: int) -> dict:
    ...
    Returns a flat dict: {config_key: (config_value, last_refreshed_at)}.
    """
...
# Header
# ---------------------------------------------------------------------------
st.header(":material/dashboard: LocID for Snowflake")
...
with btn1:
    if st.button(":material/lock: Run Encrypt", use_container_width=True,
        ...
        st.switch_page("views/run_encrypt.py")
with btn2:
    if st.button(":material/lock_open: Run Decrypt", use_container_width=True,
                 disabled=not onboarding_complete):
        st.switch_page("views/run_decrypt.py")
with btn3:
    if st.button(":material/history: Job History", use_container_width=True):
        st.switch_page("views/job_history.py")

st.divider()

# ---------------------------------------------------------------------------
# Entitlements + Recent Activity (two-column layout)
# ---------------------------------------------------------------------------
ent_col, act_col = st.columns(2)

with ent_col:
    st.subheader(":material/verified_user: Your Entitlements")
...
with act_col:
    st.subheader(":material/analytics: Recent Activity")
